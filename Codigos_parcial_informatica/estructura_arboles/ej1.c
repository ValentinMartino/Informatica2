...
int main()
{
	...
	do
	{
		...
		switch(opcion)
		{
			...
			case 2:		borrarNodo(&ptrRaiz, n);
			break;
			
			case 3:		mostrarArbol(ptrRaiz, contador);
			break;
...
void insertarNodo(ptrNodoArbol *ptrArbol, int n)
{
	...
	else // El ol ya existe
	{
		...
		{
			insertarNodo(&( (*ptrArbol) -> ptrIzq), n); // Se llama recursivamente hasta encontrar la condicion base
		}
		else 
			if (n > (*ptrArbol)->dato) // si el hijo es mayor a la ra
			{
				insertarNodo(&((*ptrArbol)->ptrDer), n); // Se llama recursivamente para crear el nodo
			}
			else
			{
